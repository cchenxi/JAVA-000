...
> 使用老师提供的 GCLogAnalysis.java 测试各种垃圾收集器
> 
> 每种场景下演示不同的堆内存配置下的GC信息，分别演示 128m -> 256m -> 512m -> 1g -> 2g -> 4g
> 
> 使用 `-XX:+PrintGCDetails -XX:+PrintGCDateStamps` 打印GC信息
...
## 2. 写一段代码，使用 HttpClient 或 OkHttp 访问 http://localhost:8801 ，代码提交到 Github。（必做）

源码见文件 io.github.cchenxi.w2.nio.client.HttpClient
